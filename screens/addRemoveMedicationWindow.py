# -----------------------------------------------------------------------------
# Name:        Add and remove medication screen (addRemoveMedication.py)
# Purpose:     Creates the screen for adding and removing medication events
#
...
class AddRemoveMedicationWindow(Screen):
    """
    A class for creating the AddRemoveMedicationWindow screen

    Allows for the creation of the AddRemoveMedicationWindow screen in the .kv file
    ...
    -------
    add() -> None
        Adds a medication to user's file
    remove() -> None
        Removes a medication from user's file
    """
    def add(self) -> None:
        """
        Adds a new medication event to the user's calendar file

        Checks for a unique medication event and adds it to the user specific calendar file
        ...
        mName = self.ids.mName.text
        mType = self.ids.mType.text
        dose = self.ids.dose.text

        # Checks for unique time and date
        if len(name) == 0 or len(time) == 0 or len(date) == 0 or len(mName) == 0 or len(str(dose)) == 0:
            exists = True
        else:
        ...
        if exists == True:
            ...
            newEvent = "m; " + name + "; " + location + "; " + note + "; " + time + "; " + date + "; " + mName + "; " + mType + "; " + str(dose) + "; \n"
            file = open(currentAccount + "calendar.txt", "a")
            file.write(newEvent)
            file.close()
    ...
    def remove(self) -> None:
        """
        Removes a medication event from the user's calendar file
